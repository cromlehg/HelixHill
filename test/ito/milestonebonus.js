...
export default function (Token, Crowdsale, wallets) {
  ...
  before(async function () {
    ...
    await crowdsale.addMilestone(ether(5000), tokens(5000), 115);
    await crowdsale.addMilestone(ether(1000), tokens(2000), 30);
    await crowdsale.addMilestone(ether(1000), tokens(1950), 30);
    await crowdsale.addMilestone(ether(2000), tokens(1800), 30);
    await crowdsale.addMilestone(ether(3000), tokens(1750), 30);
    await crowdsale.addMilestone(ether(3500), tokens(1600), 30);
    await crowdsale.addMilestone(ether(4000), tokens(1550), 30);
    await crowdsale.addMilestone(ether(4500), tokens(1500), 30);
    await crowdsale.addMilestone(ether(5000), tokens(1450), 30);
    await crowdsale.addMilestone(ether(6000), tokens(1400), 30);
    await crowdsale.addMilestone(ether(8000), tokens(1000), 30);
  ...
  milestones.forEach((milestone, i) => {
    it(`price should be ${milestone.price / 1000000000000000000} tokens per eth for milestone #${i} when hardcap is reached`, async function () {
      const investment = milestone.hardcap;
      await crowdsale.sendTransaction({value: investment, from: wallets[i+11]});
      const balance = await token.balanceOf(wallets[i+11]);
